...
const parseResumeWithClaude = async (resumeText: string, token?: string): Promise<ParsedResume> => {
    ...
    if (!response.ok) {
        ...
        throw new Error(`Backend Claude API error ${response.status}: ${errorBody}`);
    }
    ...
    return {
        ...
        technicalSkills: Array.isArray(data.data.technicalSkills) ? data.data.technicalSkills : [],
        totalExperience: typeof data.data.totalExperience === 'number' ? data.data.totalExperience : 0,
        professionalSummary: data.data.professionalSummary ?? '',
    };
};

export const parseResume = async (file: File, token?: string): Promise<ParsedResume> => {
