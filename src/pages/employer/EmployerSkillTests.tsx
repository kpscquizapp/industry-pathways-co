...
const EmployerSkillTests = () => {
  ...
  const [sessionScheduled, setSessionScheduled] = useState<Record<string, Candidate>>({});

  // Queries
  const { data: jobsResponse, isLoading: jobsLoading } = useGetEmployerJobsQuery({ page: 1, limit: 100 });
  const activeJobs = useMemo(() => {
    if (!jobsResponse?.data) return [];
    return jobsResponse.data.filter((j: any) => j.status === "active" || j.status === "published");
  }, [jobsResponse]);
  ...
  const [createCodingTest, { isLoading: isCreating }] = useCreateCodingTestMutation();
  const [sendInviteEmail, { isLoading: isInviting }] = useSendInviteEmailMutation();
  ...
  const candidatesList = useMemo<Candidate[]>(() => {
    ...
    return matchesResponse.data
      ...
      .map((match: any) => ({
        ...
        skills: Array.isArray(match.skills) ? match.skills : (typeof match.skills === 'string' ? match.skills.split(',') : []),
        status: 'available',
  ...
  const availableCandidates = fullCandidates.filter(c => c.status === 'available');
  const scheduledCandidates = fullCandidates.filter(c => c.status === 'scheduled');
  const completedCandidates = fullCandidates.filter(c => c.status === 'completed');
  ...
  const invitedCandidates = useMemo<Candidate[]>(() => {
    ...
    return matchesResponse.data
      ...
      .map((m: any) => ({
        ...
        skills: Array.isArray(m.skills) ? m.skills : (typeof m.skills === 'string' ? m.skills.split(',') : []),
  ...
  const handleScheduleTest = async () => {
    ...
    const params = new URLSearchParams({
      ...
      testType: scheduleData.testType,
      jobId: selectedJobId,
    ...
    <div className="min-h-screen bg-[#f2f5fa] px-4 sm:px-8 py-6 sm:py-8 font-sans">
      <div className="max-w-[1440px] mx-auto space-y-7">
          ...
          <div className="flex flex-col md:flex-row md:items-start justify-between gap-6 pb-2 border-b border-gray-200/50 mb-6">
            <div className="pb-4">
              <h1 className="text-2xl md:text-3xl font-extrabold tracking-tight text-gray-900">Skill Tests</h1>
              <p className="text-gray-500 mt-1.5 text-[15px]">Schedule and manage coding assessments for your candidates</p>
            </div>

            <div className="flex flex-col md:flex-row items-end gap-3 pb-4">
              <div className="relative shrink-0">
                <div className="text-[11px] font-bold text-gray-400 uppercase tracking-wider mb-1.5 ml-1">Select Job Pipeline</div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-5">
           <div 
              className="bg-white rounded-[20px] border border-gray-100 p-6 shadow-sm flex items-center justify-between hover:shadow-md transition-shadow cursor-pointer"
              onClick={() => document.getElementById('available-candidates-section')?.scrollIntoView({ behavior: 'smooth' })}
           >
              <div>
                 <p className="text-[12px] font-bold text-gray-400 uppercase tracking-wider">Available</p>
                 <p className="text-[32px] font-extrabold text-gray-900 mt-1 leading-none">{availableCandidates.length}</p>
              </div>
           ...
           {/* <div 
              className="bg-white rounded-[20px] border border-gray-100 p-6 shadow-sm flex items-center justify-between hover:shadow-md transition-shadow cursor-pointer"
              onClick={() => document.getElementById('scheduled-tests-section')?.scrollIntoView({ behavior: 'smooth' })}
           >
              <div>
                 <p className="text-[12px] font-bold text-amber-500 uppercase tracking-wider">Scheduled tests</p>
                 <p className="text-[32px] font-extrabold text-amber-500 mt-1 leading-none">{scheduledCandidates.length}</p>
              </div>
           ...
           <div 
              className="bg-white rounded-[20px] border border-gray-100 p-6 shadow-sm flex items-center justify-between hover:shadow-md transition-shadow cursor-pointer"
              onClick={() => document.getElementById('invited-candidates-section')?.scrollIntoView({ behavior: 'smooth' })}
           >
              <div>
                 <p className="text-[12px] font-bold text-violet-500 uppercase tracking-wider">Invited candidates</p>
                 <p className="text-[32px] font-extrabold text-violet-500 mt-1 leading-none">{invitedCandidates.length}</p>
              </div>
           ...
           <div 
              className="bg-white rounded-[20px] border border-gray-100 p-6 shadow-sm flex items-center justify-between hover:shadow-md transition-shadow cursor-pointer"
              onClick={() => document.getElementById('completed-tests-section')?.scrollIntoView({ behavior: 'smooth' })}
           >
              <div>
                 <p className="text-[12px] font-bold text-emerald-500 uppercase tracking-wider">Completed tasks</p>
                 <p className="text-[32px] font-extrabold text-emerald-500 mt-1 leading-none">{completedCandidates.length}</p>
              </div>
        ...
        <div className="space-y-8 pb-10">
          ...
          <Card id="available-candidates-section" className="border border-gray-200 shadow-sm rounded-[24px] bg-white overflow-hidden">
            <CardHeader className="bg-[#fcfdfa] border-b border-gray-100 px-7 py-6">
              <div className="flex items-center gap-3">
                ...
                <div>
                   <CardTitle className="text-[18px] font-bold text-gray-900 leading-none">Shortlisted Candidates</CardTitle>
                   <p className="text-[13px] text-gray-500 mt-1.5 font-medium">Select a candidate to invite them to a coding test.</p>
                </div>
              </div>
            </CardHeader>
            <CardContent className="p-0">
              {jobsLoading || matchesLoading ? (
                 <div className="flex justify-center p-14"><SpinnerLoader className="w-8 h-8 text-[#0ea5e9]" /></div>
                  ...
                  {filteredCandidates(availableCandidates).map((candidate) => (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <h3 className="font-bold text-gray-900 text-[16px] truncate">{candidate.name}</h3>
                        <p className="text-[13px] text-gray-500 font-medium truncate mt-0.5">{candidate.role}</p>
                        <div className="flex items-center gap-2 mt-2.5 flex-wrap">
                          {candidate.skills.slice(0, 4).map(skill => (
                            <span key={skill} className="px-2.5 py-1 rounded-md bg-white text-gray-600 text-[11px] font-bold border border-gray-200 shadow-sm">
                               {skill}
                            </span>
                          ))}
                        </div>
                      ...
                      {(() => {
                        ...
                        const scoreColor = score >= 90 ? "text-[#08b8cc]" : score >= 80 ? "text-[#3b82f6]" : "text-[#f59e0b]";
                        const scoreBorder = score >= 90 ? "border-[#08b8cc]" : score >= 80 ? "border-[#3b82f6]" : "border-[#f59e0b]";
                        ...
                        return (
                          <div className="flex flex-col items-center justify-center px-6 shrink-0 hidden md:flex border-l border-r border-gray-100 min-w-[120px]">
                            <div className={`w-11 h-11 rounded-full border-2 ${scoreBorder} flex items-center justify-center`}>
                              <span className={`font-extrabold text-[13px] ${scoreColor}`}>{score}%</span>
                            </div>
                            <span className="text-[10px] text-gray-400 font-bold mt-1.5 uppercase tracking-wider">AI Match</span>
                          </div>
                        );
                      })()}
                ...
                <div className="text-center py-20 text-gray-400 bg-gray-50/50">
                  ...
                  <p className="font-bold text-gray-700 text-[15px]">No shortlisted candidates</p>
                  <p className="text-[13px] mt-1.5 max-w-[280px] mx-auto text-center leading-relaxed">Go to the AI Shortlists page, select this job, and shortlist candidates to invite them here.</p>
                </div>
              )}
            </CardContent>
          </Card>
          ...
          {invitedCandidates.length > 0 && (
            <Card id="invited-candidates-section" className="border border-violet-200 shadow-sm rounded-[24px] bg-white overflow-hidden">
              <CardHeader className="bg-[#faf5ff] border-b border-violet-100 px-7 py-6">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                     <CardTitle className="text-[18px] font-bold text-gray-900 leading-none">Invited Candidates</CardTitle>
                     <p className="text-[13px] text-gray-500 mt-1.5 font-medium">Candidates who have been sent an invite for this role.</p>
                  </div>
                </div>
              </CardHeader>
              <CardContent className="p-0">
                <div className="divide-y divide-violet-50">
                  {filteredCandidates(invitedCandidates).map((candidate) => (
                    <div
                      key={candidate.id}
          ...
          {scheduledCandidates.length > 0 && (
            <Card id="scheduled-tests-section" className="border border-amber-200 shadow-sm rounded-[24px] bg-white overflow-hidden">
              <CardHeader className="bg-[#fffbeb] border-b border-amber-100 px-7 py-6">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                     <CardTitle className="text-[18px] font-bold text-gray-900 leading-none">Scheduled Test</CardTitle>
                     <p className="text-[13px] text-gray-500 mt-1.5 font-medium">Candidates who have been invited to a coding task.</p>
                  </div>
                </div>
              </CardHeader>
              <CardContent className="p-0">
                <div className="divide-y divide-amber-50">
                  {filteredCandidates(scheduledCandidates).map((candidate) => (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <h3 className="font-bold text-gray-900 text-[15px] truncate">{candidate.name}</h3>
                        <p className="text-[13px] text-gray-500 truncate mt-0.5">{candidate.role}</p>
                      </div>
                      ...
                      <div className="shrink-0 flex justify-end pl-2">
                         <Badge className="bg-amber-100 border-none text-amber-700 hover:bg-amber-100 px-3.5 py-1.5 font-bold shadow-none text-[12px]">Pending Completion</Badge>
                      </div>
                    </div>
                  ))}
                </div>
          ...
          {/* Completed Tests - Empty in this state, shown if tasks exist */}
          {completedCandidates.length > 0 && (
            <Card id="completed-tests-section" className="border border-emerald-200 shadow-sm rounded-[24px] bg-white overflow-hidden">
              <CardHeader className="bg-[#ecfdf5] border-b border-emerald-100 px-7 py-6">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <CardTitle className="text-[18px] font-bold text-gray-900 leading-none">Completed Tasks</CardTitle>
                    <p className="text-[13px] text-gray-500 mt-1.5 font-medium">Candidates who have finished their assessments.</p>
                  </div>
                </div>
              </CardHeader>
              <CardContent className="p-0">
                <div className="divide-y divide-emerald-50">
                  {filteredCandidates(completedCandidates).map((candidate) => (
                    <div key={candidate.id} className="flex items-center justify-between gap-4 p-6 hover:bg-[#f2fdf7] transition-all">
                       <div className="flex items-center gap-4">
                          ...
                          <div>
                            ...
                            <p className="text-[13px] text-gray-500 mt-0.5">{candidate.role}</p>
                          </div>
                       </div>
                       <Badge className="bg-emerald-100 text-emerald-700 hover:bg-emerald-100 px-3.5 py-1.5 font-bold text-[12px]">Passed ({candidate.testScore}%)</Badge>
                    </div>
                  ))}
                </div>
              </CardContent>
          ...
          {selectedCandidate && (
            <div className="space-y-6">
              <div className="flex items-center gap-4 p-4 rounded-xl bg-gray-50 border border-gray-100 shadow-sm">
                ...
                <div>
                  <h3 className="font-bold text-gray-900 text-[15px]">{selectedCandidate.name}</h3>
                  <p className="text-[12px] font-bold text-gray-500 mt-0.5">{selectedCandidate.role}</p>
                </div>
              </div>
              ...
              <div className="space-y-4">
                ...
                <div>
                  <Label className="text-[11px] font-bold text-gray-500 uppercase tracking-wider">Test Duration</Label>
                ...
                <div>
                  <Label className="text-[11px] font-bold text-gray-500 uppercase tracking-wider">Test Type</Label>
                  <Input 
